...
describe('ShiftypesComponent', () => {
  let component: ShiftypesComponent;
  let fixture: ComponentFixture<ShiftypesComponent>;
  ...
  beforeEach(async(() => {
    ...
    TestBed.configureTestingModule({
      ...
      declarations: [ ShiftypesComponent ],
      providers: [ ShiftypesService, NzMessageService ]
    })
    .compileComponents();
  }));

  beforeEach(() => {
    fixture = TestBed.createComponent(ShiftypesComponent);
    component = fixture.componentInstance;
    service = TestBed.inject(ShiftypesService);
  ...
  // post create apps 
  it('should call PostAllshiftypesValues and reset shiftypes properties', () => {
    // Create a spy for the Create method of the service
    spyOn(service, 'PostAllshiftypesValues').and.returnValue(of({}));
  
    // Set values for shiftypes properties
    component.shiftypes.name = 'name Name';
    component.shiftypes.description = 'description Name';

    // Call the Create method
    component.Create();

    // Expect the PostAllshiftypesValues method to have been called with the shiftypes object
    expect(service.PostAllshiftypesValues).toHaveBeenCalledWith(component.shiftypes);

    // Expect the shiftypes properties to be reset
    expect(component.shiftypes.name).toBe('');
    expect(component.shiftypes.description).toBe('');

  });
  it('should log error on update PostAllshiftypesValues failure', () => {
    const error = new Error('PostAllshiftypesValues failed');
    spyOn(service, 'PostAllshiftypesValues').and.returnValue(throwError(() => {
      return error;
    }));
    spyOn(console, 'log');
  ...
  it('should set the rowData property on successful response', () => {
    ...
    spyOn(service, 'GetAllshiftypesValues').and.returnValue(of(mockData));
  ...
  //update
  it('should clear UpdateshiftypesComponent shiftypes properties on successful update', () => {
    spyOn(service, 'Updateshiftypes').and.returnValue(of({}));
  ...
  it('should log error on update failure', () => {
    ...
    spyOn(service, 'Updateshiftypes').and.returnValue(throwError(() => {
      return error;
    }));
    spyOn(console, 'log');
  ...
  // delete the entity
  it('should call DeleteshiftypesValues and call GetAllValues on success', (() => {
    const deleteId = 123;
    const mockData:any = { 
      _id: 1, 
      name: 'test 1',
    };
    const deleteshiftypesValuesSpy = spyOn(service, 'DeleteshiftypesValues').and.returnValue(of(mockData));
    const gpGetAllValuesSpy = spyOn(component, 'GetAllValues');
    
    component.Delete(deleteId);
  

    expect(deleteshiftypesValuesSpy).toHaveBeenCalledWith(deleteId);
  ...
  it('should log error on failure', (() => {
    ...
    spyOn(console, 'log');
    spyOn(service, 'DeleteshiftypesValues').and.returnValue(throwError(error));
